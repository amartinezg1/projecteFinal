...
class UpdateClientMiddleware
{
     ...
     */
     public function handle($request, Closure $next)
     {
       ...
       if(self::validateDni($dni) && self::validateEmail($email) && self::validateZipCode($zip_code) && self::validatePhones($phone1)){
         return $next($request);
       }
     ...
     {
       if(preg_match("/^(?:0[1-9]|[1-4]\d|5[0-2])\d{3}$/",$zip_code) == 1){
         return true;
       }else{
         return false;
       }
